  ...
  try {
    ...
    if (customerId) {
        if (!mongoose.Types.ObjectId.isValid(customerId)) {
             return NextResponse.json({ message: 'Invalid customer ID format in query parameter.' }, { status: 400 });
        }
    ...
    console.error('GET Loans API error:', error);
     ...
     if (error.message.includes('Could not connect to database')) {
      ...
      if (error instanceof mongoose.Error) { // Catch other Mongoose errors during find/populate
        ...
        return NextResponse.json({ message: 'Database error fetching loans.' }, { status: 500 });
     }
    // Generic error
    return NextResponse.json({ message: 'Server error fetching loans', error: error.message || 'Unknown error' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!customer) {
        return NextResponse.json({ message: 'Customer not found or does not belong to this shopkeeper.' }, { status: 404 }); // 404 Not Found or 403 Forbidden
    }
    ...
    // The pre-save middleware will set balance and initial status
    ...
    console.error('POST Loan API error:', error);
     ...
     if (error instanceof mongoose.Error.CastError) { // Invalid data type (e.g., ObjectId)
          return NextResponse.json({ message: `Invalid data type provided for field ${error.path}. Expected ${error.kind}.` }, { status: 400 });
     }
     if (error instanceof mongoose.Error) { // Catch other Mongoose errors during find/save
        console.error('Mongoose error creating loan:', error);
        return NextResponse.json({ message: 'Database error creating loan.' }, { status: 500 });
     }
    // Generic error
    return NextResponse.json({ message: 'Server error creating loan', error: error.message || 'Unknown error' }, { status: 500 });
  }
}
