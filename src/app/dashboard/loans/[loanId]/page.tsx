...
export default function LoanDetailPage() {
  ...
  const fetchLoanDetails = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            `Failed to fetch loan details (${response.status})`
        );
      }
  ...
  const handleRepaymentInputChange = (
  ...
  const handleRecordRepayment = async (e: React.FormEvent) => {
    ...
    if (amountNum > loan.balance + tolerance) {
      setFormError(
        `Repayment amount (₹${amountNum.toFixed(
          2
        )}) cannot exceed the current balance (${formatCurrency(
          loan.balance
        )}).`
      );
      setIsSubmitting(false);
    ...
    try {
      ...
      setFormError(
        err.message || "An unknown error occurred while recording repayment."
      );
    } finally {
  ...
  const getStatusBadgeVariant = (
    ...
    switch (status) {
      ...
      case "overdue":
        return "destructive";
      case "pending":
        return "default"; // Using primary theme color
      default:
        return "outline";
    }
  };
      ...
      <div className="container mx-auto py-6 px-4 md:px-6 text-center space-y-4">
        ...
        <Card className="p-10 shadow-md rounded-lg border">
          ...
          <p className="text-muted-foreground mt-2">
            The requested loan details could not be loaded or the loan does not
            exist.
          </p>
        </Card>
      </div>
  ...
  const daysOverdue =
    loan.status === "overdue"
      ? calculateDaysOverdue(loan.dueDate, loan.graceDays)
      : 0;
    ...
    <div className="container mx-auto py-6 px-4 md:px-6 space-y-8 print:space-y-4">
      ...
      <Card className="shadow-md rounded-lg border border-border print:shadow-none print:border-0">
        ...
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b print:border-b-0 print:pb-2">
          ...
          <div>
            <CardTitle className="text-xl lg:text-2xl mb-1 font-semibold text-foreground">
              {loan.description}
            </CardTitle>
            <div className="text-sm text-muted-foreground space-y-1 mt-2">
              <div className="flex items-center gap-2">
                ...
                <Link
                  href={`/dashboard/customers?id=${loan.customer._id}`}
              ...
              {loan.customer.address && (
                <div className="flex items-center gap-2">
                  <HomeIcon className="h-4 w-4 flex-shrink-0 text-primary" />{" "}
          ...
          <div className="flex flex-col items-start sm:items-end gap-2 mt-2 sm:mt-0">
            <Badge
              ...
              className="text-base px-4 py-1.5 rounded-full shadow-sm"
            >
        ...
        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 print:grid-cols-4">
          ...
          <div className="flex flex-col items-center text-center p-3 rounded-md bg-muted/40 border">
            ...
            <p
              className={cn(
                ...
                loan.balance > 0 ? "text-orange-600" : "text-green-600"
              )}
            >
              <IndianRupee className="h-4 w-4 mr-0.5" />{" "}
          ...
          <div className="flex flex-col items-center text-center p-3 rounded-md bg-muted/40 border">
            ...
            <p className="text-xl font-semibold flex items-center">
              <Percent className="h-4 w-4 mr-0.5" /> {loan.interestRate}%{" "}
              {loan.interestRate > 0 ? (
                <span className="text-xs ml-1">(Annual)</span>
              ) : (
                ""
              )}
            </p>
        ...
        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-0 pb-6 print:grid-cols-4 print:pb-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm font-medium">
              {isValid(dueDateParsed)
                ? format(dueDateParsed, "PPP")
                : "Invalid Date"}
            </p>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm font-medium">
              {loan.graceDays > 0
                ? `${loan.graceDays} day${loan.graceDays !== 1 ? "s" : ""}`
                : "None"}
            </p>
          </div>
        </CardContent>
        ...
        <Card className="shadow-md rounded-lg border border-border print:hidden">
          ...
          <CardContent>
            ...
            >
              ...
              <div className="space-y-2">
                <Label htmlFor="date">
                  Repayment Date <span className="text-destructive">*</span>
                </Label>
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        !repaymentForm.date && "text-muted-foreground"
                      )}
                      disabled={isSubmitting}
                    >
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {repaymentForm.date ? (
                        format(repaymentForm.date, "PPP")
                      ) : (
                        <span>Pick date</span>
                      )}
                    </Button>
              ...
              <Button
                type="submit"
                disabled={
                  isSubmitting || !repaymentForm.amount || !repaymentForm.date
                }
      ...
      <Card className="shadow-md rounded-lg border border-border print:shadow-none print:border">
        <CardHeader>
          ...
          <CardDescription>
            List of payments recorded for this loan.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          {loan.repayments.length === 0 ? (
            // Empty state for repayments
            <p className="text-muted-foreground text-center py-10 px-6 bg-muted/30">
              ...
              <Table>
                ...
                <TableBody>
                  {loan.repayments
                    ...
                    ) // Ensure descending sort by repayment date
                    .map((repayment) => {
                      ...
                      return (
                        ...
                        >
                          <TableCell className="pl-6">
                            {isValid(repaymentDateParsed)
                              ? format(repaymentDateParsed, "PPP")
                              : "Invalid Date"}
                          </TableCell>
                          <TableCell className="text-right font-medium text-green-600 pr-6">
