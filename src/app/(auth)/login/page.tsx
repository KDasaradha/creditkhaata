  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        ...
        const redirectPath =
          sessionStorage.getItem("loginRedirect") || "/dashboard";
        sessionStorage.removeItem("loginRedirect"); // Clean up storage
    ...
    <Card className="w-full shadow-xl border-t-4 border-primary rounded-lg overflow-hidden animate-in fade-in duration-500">
      ...
      <CardContent className="px-6 py-6 space-y-6">
        ...
        {successMessage && (
          <Alert
            variant="default"
            className="bg-green-50 border-green-300 dark:bg-green-900/30 dark:border-green-700/50"
          >
      ...
      <CardFooter className="flex flex-col items-center text-sm bg-muted/50 py-5 border-t">
        ...
        <Link
          href="/register"
          className="font-semibold text-primary hover:underline hover:text-primary/80 transition-colors mt-1"
        >
          Register here
        </Link>
      </CardFooter>
